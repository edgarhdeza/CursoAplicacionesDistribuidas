...
{
	internal class RemoveRoleCommandHandler : IRequestHandler<RemoveRoleCommand, Result<string>>
	{
		private readonly UserManager<IdentityUser> userManager;
		private readonly RoleManager<IdentityRole> roleManager;

		public RemoveRoleCommandHandler(UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager)
		{
		...
		public async Task<Result<string>> Handle(RemoveRoleCommand request, CancellationToken cancellationToken)
		{
			...
			var result = await userManager.RemoveFromRoleAsync(existUser, existRole.Name!);

			if (!result.Succeeded)
			{
				return await Result<string>.FailAsync("Error al remover role");
			}

			return await Result<string>.SuccessAsync("Role borrado", "");
		}
	}
}
